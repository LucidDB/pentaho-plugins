...
public class WHLabelServiceImplLite extends RemoteServiceServlet implements WHLabelService {
	...
	public int setDWLabel(String dsName, String labelName) {
		...
		try {
			...
			Logger.error(getClass().toString(), "setDWLabel --> value set to the Session is --->"+ labelName );
			getDWLabel();
		} catch (Exception e) {
			Logger.error(getClass().toString(), "Error while Setting DataWarehouse Labels to session... "+ labelName + " ----- "+e);
		}
		return result;
	}
